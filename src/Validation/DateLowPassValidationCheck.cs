...
/// </summary>
public class DateLowPassValidationCheck : ValidationCheck
{
	#region Members
	...
	/// Default constructor.
	/// </summary>
	public DateLowPassValidationCheck(DateTime cutOffDate) :
		base("After Specified Date")
	{
	...
	protected override bool ValidityCheck(List<string> entries, List<EntryTranslationMetaData> entryMetaData, RecordTranslationMetaData recordMetaData, TableTranslationMetaData tableMetaData)
	{
		for (int i = 0; i < entries.Count; i++)
		{
			...
			{
				if (Validation.TryParseDate(entries[i], out DateTime dateTime))
				{
