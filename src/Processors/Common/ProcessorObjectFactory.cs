...
public static class ProcessorObjectFactory
{
	...
	private static readonly ObjectFactory<string, InputProcessor>			_inputProcessorFactory			= new();
	private static readonly ObjectFactory<string, OutputProcessor>			_outputProcessorFactory			= new();
	...
	private static void RegisterInputProcessors()
	{
		...
		MethodInfo? method = typeof(ObjectFactory<string, InputProcessor>).GetMethod("Register", new Type[] {typeof(string)}) ??
			throw new NullReferenceException("Failed to get registration method.");
	...
	{
		List<ProcessorMetaData> processorMetaDataList = new(processorTypes.Count);
		for (int i = 0; i < processorTypes.Count; i++)
		{
			ProcessorMetaData? processorMetaData = DigitalProduction.Reflection.Attributes.GetAttribute<ProcessorMetaData>(processorTypes[i]) ??
				throw new NullReferenceException("The processor meta data does not exist.");

			processorMetaDataList.Add(processorMetaData);
		}
		return processorMetaDataList;
	}
